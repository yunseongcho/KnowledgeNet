...
논문을 이해하거나, 요약 및 문서화 할 때 조금 편해보고자 만들었습니다. Gemini-2.0-flash 모델을 사용하여 논문과 논문에 관한 프롬프트들을 Chat 형식으로 입력하여 논문에 대한 요약, 설명이 포함된 Markdown 파일을 만듭니다.
...
- `configs/gemini-2.0-flash.json`: Configuration file for the models(explainer, translator).
...
- explainer 와 translator 두 모델의 configuration 은 `configs/gemini-2.0-flash.json` 을 참조하십시오. `model_name`, `temperature`, `top_p` 등을 변경할 수 있습니다.
...
- 간혹 `recitation error` 혹은 `pdf merge error` 가 발생하는 경우가 있습니다.
    - `recitation error` 는 gemini가 이전 답변과 아주 유사한 답변을 생성했을 때 발생하는 에러입니다. 현재로서는 해결방법이 없으며, seed 나 temperature 등을 변경하여 다시 돌려봐야합니다.
    - `pdf merge error` 의 경우 pypdf 에서 main paper와 supplemental 을 merge 하지 못한 경우 발생합니다. 다른 tool을 사용하여 수동으로 머지시켜주면 됩니다.
    - 두 에러가 발생한 경우 root 디렉토리에 `~~error.txt` 파일에 에러가 난 논문을 logging 하도록 해두었습니다.
## License
